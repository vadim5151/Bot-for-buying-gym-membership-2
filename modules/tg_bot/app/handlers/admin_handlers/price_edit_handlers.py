...
async def editing_price(message: Message):
    ...
    temp_message = await message.answer(
        generate_price_text(prices), 
        reply_markup=kb.create_admin_price_list(months)
    )
...
async def update_price_btn(callback: CallbackQuery, state: FSMContext):
    ...
    await user_repo.add_temp_message_id(callback.from_user.id, temp_message.message_id)
    await callback.answer('')
...
async def update_price(message: Message, state: FSMContext):
    ...
    if await price_repo.get_by_month(month):
        ...
        temp_message = await message.answer(
            AdminPrice.PRICE_UPDATED.format(old_price=old_price_text, new_price=new_price_text)
        ) 
    else:
    ...
    await user_repo.add_temp_message_id(message.from_user.id, temp_message.message_id)
    await user_repo.delete_temp_messages(message.from_user.id, message.chat.id, bot)
...
async def remove_price(callback: CallbackQuery, state: FSMContext):
    ...
    temp_message = await callback.message.edit_text(
        AdminPrice.PRICE_DELETED.format(price=deleted_price)
    )
