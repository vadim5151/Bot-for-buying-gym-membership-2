...
async def handle_custom_period_text(message: Message, state: FSMContext):
    ...
    if start_period > end_period:
        ...
        await user_repo.add_temp_message_id(message.from_user.id, temp_message.message_id)
        await user_repo.add_temp_message_id(message.from_user.id, message.message_id)
        return 
...
async def cancel_select(callback: CallbackQuery, state: FSMContext):
    ...
    await user_repo.delete_temp_messages(callback.from_user.id, callback.message.chat.id, bot)
    await state.clear()
